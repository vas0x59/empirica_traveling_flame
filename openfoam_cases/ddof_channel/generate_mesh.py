...
def gmsh_generate_domain(d_w, d_h, c_w, c_h, l, l_cnt, s1=1):
    ...
    s_d = s1*10
    points_cc = []
    points_cc.append(model.geo.addPoint(0, c_w/2, 0, s_ch)) # rb
    points_cc.append(model.geo.addPoint(0, c_w/2, c_h, s_ch)) # rt
    points_cc.append(model.geo.addPoint(0, -c_w/2, c_h, s_ch)) # lt
    points_cc.append(model.geo.addPoint(0, -c_w/2, 0, s_ch)) # lb
    ...
    for i in [1, 3]:
        model.geo.mesh.setTransfiniteCurve(lines_cc[i] , int(40))# l, r
    ...
    lines_ed = []
    points_ed = []
    points_ed.append(model.geo.addPoint(0, d_w/2, c_h, s_d)) # rb
    points_ed.append(model.geo.addPoint(0, d_w/2, d_h+c_h, s_d)) # rt
    points_ed.append(model.geo.addPoint(0, -d_w/2, d_h+c_h, s_d)) # lt
    points_ed.append(model.geo.addPoint(0, -d_w/2, c_h, s_d)) # lb
    ...
    model.geo.mesh.setTransfiniteCurve(lines_ed[1] , int(6), "Progression", 1.2 )# l, r
    model.geo.mesh.setTransfiniteCurve(lines_ed[3] , int(6), "Progression", -1.2 )# l, r
    ...
    ov = model.geo.extrude([(2, surface_cc), (2, surface_ed)], l, 0, 0, [l_cnt], [], recombine=True)
    ...
    model.addPhysicalGroup(2, start, name="border_0") # boundary
    model.addPhysicalGroup(2, end, name="XL") # boundary
    model.addPhysicalGroup(2, air, name="top") # boundary
    model.addPhysicalGroup(2, [ov[2][1]], name="border_liquid") # boundary
    model.addPhysicalGroup(2, [ov[3][1],  ov[5][1]], name="border") # boundary
...
if __name__ == "__main__":
    ...
    model = gmsh_generate_domain(d_w=0.015, d_h=0.02, c_w=10e-3,c_h=20e-3, l = 0.1, l_cnt = 180)
    # os.makedirs("workdir", exist_ok=True)
    gmsh.write(f"mesh.msh2")
